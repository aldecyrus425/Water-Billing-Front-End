...
import { CreateReadingModal } from '../../components/modals/reading/addModal';

const Readings = () => {
  const [createReadingModal, setCreateReadingModal] = useState(false);
  ...
  return (
    <>
        ...
        <div className='bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-b-2xl border border-slate-200/50 dark:border-slate-700/50 overflow-hidden'>
          <div className='p-6 border-b border-slate-200/50 dark:border-slate-700/50 space-y-5'>
            <div className='flex items-center justify-between'>
              <div>
                <h3 className='text-lg font-bold text-slate-800 dark:text-white'>Readings</h3>
                <p className='text-sm text-slate-500 dark:text-slate-400'>List's of all readings</p>
              </div>
              <button className='border rounded-lg p-3 text-blue-600 hover:text-white hover:bg-blue-600 text-sm font-medium cursor-pointer' onClick={() => setCreateReadingModal(true)}>Add Reading</button>
            </div>
            <div className='overflow-y-scroll'>
              <table className='w-full max-h-170'>
                <thead>
                  <tr>
                    <th className='text-left p-4 text-sm font-semibold text-slate-600'>Consumers ID</th>
                    ...
                    <th className='text-left p-4 text-sm font-semibold text-slate-600'>Current Reading</th>
                    <th className='text-left p-4 text-sm font-semibold text-slate-600'>Previous Reading</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className='border-b border-slate-200/50 dark:border-slate-700/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors'>
                    ...
                    <td className='p-4'>
                      <span className='text-sm text-slate-800 dark:text-white'>Current Reading</span>
                    </td>
                    <td className='p-4'>
                      <span className='text-sm text-slate-800 dark:text-white'>Previous Reading</span>
                    </td>
                  </tr>
                </tbody>
              </table>
      ...
      <CreateReadingModal isVisible={createReadingModal} onClose={() => setCreateReadingModal(false)} />
    </>
  )
}
